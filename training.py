...
def T5Trainer(
    train_set, dev_set, source_text, target_text, model_params, output_dir="./outputs/"
):
...
# this section exists to figure out the max source/target lengths
import json
...
tokenized_inputs = concatenate_datasets([dataset["train"], dataset["test"]]).map(lambda x: tokenizer(x["prompt"], truncation=True), batched=True, remove_columns=["prompt", "derivation"])
max_source_length = max([len(x) for x in tokenized_inputs["input_ids"]])
print(f"Max source length: {max_source_length}")

# The maximum total sequence length for target text after tokenization.
# Sequences longer than this will be truncated, sequences shorter will be padded."
tokenized_targets = concatenate_datasets([dataset["train"], dataset["test"]]).map(lambda x: tokenizer(x["derivation"], truncation=True), batched=True, remove_columns=["prompt", "derivation"])
...
model_params = {
    ...
    "MAX_SOURCE_TEXT_LENGTH": max_source_length,  # max length of source text
    "MAX_TARGET_TEXT_LENGTH": max_target_length,  # max length of target text
